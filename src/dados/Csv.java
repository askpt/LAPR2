...
public class Csv extends JComponent implements Accessible {
		...
		try {
			...
			while ( in.hasNextLine() ) {
				...
				for ( int i = 0 ; i < modalidades.size() ; i++ ) {
					if ( tempModal[ 0 ].equals( modalidades.get( i ).getNome() ) ) temModal = true;
				}

				if ( ! temModal ) modalidades.add( new Modalidade( tempModal[ 0 ] ) );
			}
			in.close();
			...
			while ( in2.hasNextLine() ) {
				...
				for ( ; i < modalidades.size() ; i++ ) {
					if ( temp[ 0 ].equals( modalidades.get( i ).getNome() ) ) break;
				}
				...
				if ( ! tempDisc2.getNome().equals( "temp" ) ) disciplina.add( tempDisc2 );
			}
			in2.close();
			JOptionPane.showMessageDialog( janela , "File imported successful!" , "Import File" , JOptionPane.INFORMATION_MESSAGE );
			...
			for ( int i = 0 ; i < modalidades.size() ; i++ ) {
				...
				for ( int j = 0 ; j < discTemp.size() ; j++ ) {
					...
					if ( discTemp.get( j ).getTipoClass() == 0 )
						...
						typeClass = "points";
					else if ( discTemp.get( j ).getTipoClass() == 3 ) typeClass = "rank";
					if ( discTemp.get( j ).getGenero() == 0 )
						men = "X";
					else if ( discTemp.get( j ).getGenero() == 1 )
						women = "X";
					else if ( discTemp.get( j ).getGenero() == 2 ) mixed = "X";
					...
					for ( int k = j + 1 ; k < discTemp.size() ; k++ ) {
						if ( discTemp.get( j ).equals( discTemp.get( k ) ) ) {
							if ( discTemp.get( k ).getGenero() == 0 )
								...
								women = "X";
							else if ( discTemp.get( k ).getGenero() == 2 ) mixed = "X";
							discTemp.remove( k );
							k-- ;
						}
					}
	...
	public void importProvas( Component janela , ListaLigada < Atleta > atletas , ListaLigada < Modalidade > modalidades , ListaLigada < ProvaInd > provas ) {
	...
	private class CsvFilter extends FileFilter {
		...
		public boolean accept( File f ) {
			...
			if ( extensao != null ) {
				if ( extensao.equals( "csv" ) ) return true;
			} else
				return false;
